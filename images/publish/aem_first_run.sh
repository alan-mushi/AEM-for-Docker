...
/examine_log.sh /opt/aem/${CQ_RUNMODE}/crx-quickstart/logs/stderr.log "Quickstart started"
...
cat /tmp/processes | grep "java.*-jar.*cq-${CQ_RUNMODE}-p4503.jar" | awk '{print $1}' | xargs kill -2
rm /tmp/processes
